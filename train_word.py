# Training this with only one keyword from each sentence, input as well should be one word
...
class ClipCocoDataset(Dataset):
    ...
    def __init__(self, data_root: str, data_path: str, gpt2_type: str = "gpt2", len_seq=20, len_key=5, normalize_prefix=False):
...
def train(model, epoch, train_dataloader, optimizer, lr_scheduler, scaler, args,
    ...
    for idx, (tokens, mask, prefix, gt, kt) in enumerate(train_dataloader):
        ...
        # xt = torch.cat((kt, xt), dim=1)
        # tokens = torch.cat((kt, tokens), dim=1)
